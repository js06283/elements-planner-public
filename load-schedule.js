// Load and display the real Elements Festival schedule
class ScheduleLoader {
	constructor() {
		this.parser = new FestivalScheduleParser();
		// Default CSV file path
		this.csvPath = "Elements_Festival_Full_Schedule__All_Days_.csv";
	}
		...
		if (festivalGrid) {
			festivalGrid.innerHTML = `
				<div class="error-placeholder">
					...
					<p>Unable to load the Elements Festival schedule. Please check that the CSV file is available and try refreshing the page.</p>
					<button onclick="location.reload()" class="btn-primary">Retry</button>
				</div>
			`;
		}
